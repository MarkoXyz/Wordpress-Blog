...
<div id="comments" class="comments-area">
    ...
    $fields=array(
            ...
            'code'=>'<div class="comment-form-url form-group has-feedback"><div class="input-group"><div class="input-group-addon"><i class="fa fa-key"></i></div><input class="form-control" placeholder="'.__('验证码：','moedog').$comment_num1.' ＋ '.$comment_num2.' = ?" id="code" name="code" type="text" value="" autocomplete="off" size="30" /><span class="form-control-feedback required">*</span><input type="hidden" name="co_num1" value="'.($comment_num1 + 1).'" /><input type="hidden" name="co_num2" value="'.($comment_num2 + 2).'" /></div></div>',
            'url'=>'<div class="comment-form-url form-group has-feedback"><div class="input-group"><div class="input-group-addon"><i class="fa fa-link"></i></div><input class="form-control" placeholder="'.__('网站链接','moedog').'" id="url" name="url" type="text" value="'.esc_attr($commenter['comment_author_url']).'" size="30" /></div></div>',
            'cookies'=>'',
    );
    $args=array(
        ...
        'comment_notes_before' => '<p class="comment-notes">'.__('邮箱地址不会被公开，必填项已用 * 标注','moedog').'</p>',
        'comment_field'=>'<div class="comment form-group has-feedback"><div class="input-group"><textarea class="form-control" id="comment" placeholder="'.__('说点什么吧~','moedog').'" name="comment" rows="5" aria-required="true" required  onkeydown="if(event.ctrlKey){if(event.keyCode==13){document.getElementById(\'submit\').click();return false}};"></textarea></div><div class="OwO"></div></div>',
    );
    comment_form($args); ?>
</div>